...
@export var maximum_height: float = 999
...
func _rotate_right() -> void:
	position.y = clampf(position.y + 1, minimum_height, maximum_height)

func _rotate_left() -> void:
	position.y = clampf(position.y - 0.05, minimum_height, maximum_height)
